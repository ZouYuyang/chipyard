...
class WithArty100TUARTTSI extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: UARTTSIPort, chipId: Int) => {
    ...
    val packagePinsWithPackageIOs = Seq(
      ("C4" , IOPin(harnessIO.rxd)),
      ("D4", IOPin(harnessIO.txd)))
...
class WithArty100TPMODUART extends WithArty100TUART("J4", "E6")
...
class WithArty100TJTAG extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: JTAGPort, chipId: Int) => {
    ...
    val packagePinsWithPackageIOs = Seq(
      ("H4", IOPin(harnessIO.TCK)),
      ("H1", IOPin(harnessIO.TMS)),
      ("G1", IOPin(harnessIO.TDI)),
      ("G3", IOPin(harnessIO.TDO))
    )
    packagePinsWithPackageIOs foreach { case (pin, io) => {
      ath.xdc.addPackagePin(io, pin)
      ath.xdc.addIOStandard(io, "LVCMOS33")
